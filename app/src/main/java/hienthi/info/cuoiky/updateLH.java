...
public class updateLH extends AppCompatActivity {
    ...
    EditText edttenL, edtsiso, edtgvcn, edtgvtoan, edtgvly, edtgvhoa;
    ImageView imgAvaLh;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_update_lh);
    ...
    private void initUI() {
        ...
        malh = intent.getStringExtra("MaL");
        SQLiteDatabase database = Database.initDatabase(this,DATABASE_NAME);
        Cursor cursor = database.rawQuery("Select * from LopHoc where MaL = ? ", new String[]{malh});
        cursor.moveToFirst();
        ...
        Bitmap bitmap = BitmapFactory.decodeByteArray(anhl, 0 , anhl.length);
        imgAvaLh.setImageBitmap(bitmap);
        edttenL.setText(siso);
        edtsiso.setText(gvcn);
        edtgvcn.setText(gvtoan);
        edtgvtoan.setText(gvly);
        edtgvly.setText(gvhoa);
        edtgvhoa.setText(gvhoa);
    }
    ...
    private void update(){
        ...
        byte[] anhl = getByArrayFromImageView(imgAvaLh);
        ...
        contentValues.put("AnhL",anhl);

        SQLiteDatabase database = Database.initDatabase(this,"QL_TruongHoc.sqlite");
        database.update("NhanVien",contentValues, "MaL = ?",new String[]{malh});
